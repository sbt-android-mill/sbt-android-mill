...
object Emulator extends Target with Test {
  ...
  val settings: Seq[Project.Setting[_]] = targetSettings ++ Seq(
    ...
    emulatorStageCore <<= emulatorStageCore dependsOn (emulatorStagePrepare, emulatorStageReinstall),
    emulatorStageFinalizer <<= stageFinalizerTask,
    emulatorStageFinalizer <<= emulatorStageFinalizer dependsOn (emulatorStagePrepare, emulatorStageCore, emulatorStageReinstall),
    emulatorStageReinstall <<= targetStageReinstall(true),
    emulatorStageReinstall <<= emulatorStageReinstall dependsOn (emulatorStagePrepare),
